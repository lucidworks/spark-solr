...
public class UseML implements SparkApp.RDDProcessor {
  ...
  public int run(SparkConf conf, CommandLine cli) throws Exception {
    ...
    Object bestModel = cvModel.bestModel();
    if (bestModel != null) {
      System.out.println(">> bestModel is a: " + bestModel.getClass().getName() + ": " + bestModel);
    } else {
    ...
    for (Row next : rows) {
      ...
      DataFrame scored = cvModel.transform(oneRowDF);
      Row[] scoredRows = scored.collect();
      int labelIdx = scoredRows[0].fieldIndex("predictedLabel");
      String predictedLabel = scoredRows[0].getString(labelIdx);
