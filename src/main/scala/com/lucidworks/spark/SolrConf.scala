...
class SolrConf(config: Map[String, String]) {
  ...
  def getZkHost: Option[String] = {
    if (config.contains(SOLR_ZK_HOST_PARAM))
      return config.get(SOLR_ZK_HOST_PARAM)
    None
  }

  def getCollection: Option[String] = {
    if (config.contains(SOLR_COLLECTION_PARAM))
      return config.get(SOLR_COLLECTION_PARAM)
    None
  }

  def getQuery: Option[String] = {
    if (config.contains(SOLR_QUERY_PARAM))
      return config.get(SOLR_QUERY_PARAM)
    None
  }

  def getFields: Array[String] = {
    if (config.contains(SOLR_FIELD_PARAM)) {
      if (config.get(SOLR_FIELD_PARAM).isDefined) {
  ...
  def getRows: Option[Int] = {
    if (config.contains(SOLR_ROWS_PARAM)) {
      if (config.get(SOLR_ROWS_PARAM).isDefined)
  ...
  def splits: Option[Boolean] = {
    if (config.contains(SOLR_DO_SPLITS)) {
      if (config.get(SOLR_DO_SPLITS).isDefined)
  ...
  def getSplitField: Option[String] = {
    if (config.contains(SOLR_SPLIT_FIELD_PARAM))
      return config.get(SOLR_SPLIT_FIELD_PARAM)
    None
  }

  def getSplitsPerShard: Option[Int] = {
    if (config.contains(SOLR_SPLITS_PER_SHARD_PARAM)) {
      if (config.get(SOLR_SPLITS_PER_SHARD_PARAM).isDefined)
  ...
  def escapeFieldNames: Option[Boolean] = {
    if (config.contains(ESCAPE_FIELDNAMES_PARAM)) {
      if (config.get(ESCAPE_FIELDNAMES_PARAM).isDefined)
