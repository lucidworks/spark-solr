...
==== time_series_partition_on

Set this option as true, in order to query mutiple time series collections, partitioned according to some time period

Usage: `option("time_series_partition_on", "true")`
...
Usage: `option("time_stamp_option", "ts")`
...
This option is used to specify the maximum number of partitons that must be allowed at a time.

Usage: `option("max_active_partitons", "100")`
