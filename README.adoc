...
[source]
cd $SPARK_HOME
./bin/spark-shell --packages "com.lucidworks.spark:spark-solr:2.3.3"
                          or
./bin/spark-shell --jars spark-solr-2.3.3.jar
...
`${VERSION}` will be something like 2.3.3-SNAPSHOT, for development builds.
...
You can use the `sort` option to specify how to sort results.

Usage: `option("sort","id asc")`
...
==== Other Options to Fine-tune Reads from Solr
...
The `exclude_fields` option allows you to exclude fields by name or regular expression when not using the `fields` option. When you don't specify the fields to fetch from Solr using the `fields` option, then all stored fields are retrieved. The `exclude_fields` allows you to exclude specific fields instead of listing out a large number of fields to include.

Usage: `option("exclude_fields", "id,_version_,\\d_coordinate,_raw_content_")`

Default: None

==== expr

The `expr` option allows you to specify a Solr streaming expression to execute instead of a Solr query.

Usage: `option("expr", "hashJoin(search(ratings, q=*:*, qt=\"/export\", fl=\"user_id,movie_id,rating\", sort=\"movie_id asc\", partitionKeys=\"movie_id\"), hashed=search(movies, q=*:*, fl=\"movie_id,title\", qt=\"/export\", sort=\"movie_id asc\",partitionKeys=\"movie_id\"),on=\"movie_id\")")`

Default: None

==== expr_schema

The `expr_schema` option allows you to specify the exact schema for a Solr streaming expression. By default, the library tries to determine the schema of the expression result but this option allows you to override that decision-making process.

Usage: `option("expr_schema", "name:string,avg_rating:double,total_sum:long")`

Default: None

==== sql

The `sql` option allows you to specify a Solr SQL query to execute instead of a Solr query.

Usage: `option("sql", "select movie_id from movies where _query_='plot_txt_en:dogs'")`
...
If your Solr collections are partitioned by time, then the following options can help you read documents across
multiple collections for a desired time-range.
